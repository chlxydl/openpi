...
def make_policy():
    ...
    # path_ckpt = "/root/workspace/main/AgiBot-World/checkpoints/pi0_8w_20stepspretrained_model" 
    ...
    while rclpy.ok():
        ...
        if (
            ...
            if abs(ts_h - last_ts_h) < 1e-3:
                ...
                continue
            # 如果当前推理帧和实时处理时间差异过大，跳过
            if abs(ts_h - sim_time) > 1:
                print("[INFO] frame skipping...")
                continue
            last_ts_h = ts_h
            ...
            action = policy.select_action_remote(observation)
            # sim_ros_node.publish_joint_command_16(action)
            step_count = 0
            while policy._action_queue:
                a = policy._action_queue.popleft()
                print("\n",{a})
                sim_ros_node.publish_joint_command_16(a)
                step_count += 1

                print("step_count:",(step_count))
                time.sleep(0.12)  # 暂停,等待模拟器处理,控制发布频率
                # if step_count>9:
                #     while policy._action_queue:
                #         a = policy._action_queue.popleft()
