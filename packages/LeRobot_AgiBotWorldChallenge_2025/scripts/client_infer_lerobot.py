...
class ClientPolicy:
    ...
    def select_action(self, observation):
        ...
        action_queue = response["action_queue"]  # 注意：拿到的是一个队列
        action_tensors = [torch.tensor(a, dtype=torch.float32) for a in action_queue]

        return action_tensors  # 返回整个动作队列
...
def infer(cfg):
    ...
    while rclpy.ok():
        ...
        if (
            ...
            state_tensor = torch.tensor(act_raw, dtype=torch.float32).unsqueeze(0)
            observation = {
            ...
            action_queue = policy.select_action(observation)

            step_count = 0
            for action in action_queue:
                sim_ros_node.publish_joint_command_16(action)
                step_count += 1
                # time.sleep(0.004)  # 暂停 1 秒
                print("step_count:",(step_count))
                time.sleep(0.12)  # 暂停,等待模拟器处理
                # time.sleep(0.001)  # 暂停,等待模拟器处理

                # rclpy.spin_once(sim_ros_node, timeout_sec=1)  # 
