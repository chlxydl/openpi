...
class Normalize(nn.Module):
    ...
    def __init__(
        ...
        print("🧩 [Normalize] All features init:", self.features)
        ...
        print(f"[DEBUG] : aaa {self.features}")
    ...
    def forward(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        for key, ft in self.features.items():
            if key not in batch:
                ...
                print(f"[DEBUG]:{key} not in batch")
                continue
            ...
            if norm_mode is NormalizationMode.MEAN_STD:
                ...
                ###################################################
                # 针对 std 可能为 0 的位置做掩码处理，防止除 0 或 nan
                safe_std = std.clone()
                safe_std[safe_std < 1e-5] = 1.0
                batch[key] = (batch[key] - mean) / (safe_std + 1e-8)
                ###################################################
                ...
                #########################
                range_val = max - min
                safe_range = range_val.clone()
                safe_range[safe_range < 1e-5] = 1.0
                batch[key] = (batch[key] - min) / (safe_range + 1e-8)
                #########################
                # batch[key] = (batch[key] - min) / (max - min + 1e-8)                 # normalize to [0,1]
    ...
    def forward(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        for key, ft in self.features.items():
            ...
            if norm_mode is NormalizationMode.MEAN_STD:
                ...
                safe_std = std.clone()
                safe_std[safe_std < 1e-5] = 1.0
                batch[key] = batch[key] * safe_std + mean
                ########################################
                ...
                batch[key] = batch[key] * safe_range + min
                ########################################
                # batch[key] = (batch[key] + 1) / 2
