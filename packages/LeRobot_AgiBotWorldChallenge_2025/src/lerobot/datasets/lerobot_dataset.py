...
class LeRobotDataset(torch.utils.data.Dataset): 
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        def tr(x):
            if x == 'actions':
                print("Replace key 'actions' with 'action'")
                return 'action'
            return x
        return {
            key: torch.stack(self.hf_dataset.select(q_idx)[tr(key)])
    ...
    def __getitem__(self, idx) -> dict:
        # print(self.hf_dataset[0]["action"].shape)  # 看单条样本的原始动作 shape
        ...
        #ybg: remove key" |"
        # item["task"] = self.meta.tasks[task_idx].rstrip(" |")
        # ybg:Add episode index
        # ------------------- 获得每一帧的动作文本 -------------------

        # if hasattr(self.meta, "episodes") and ep_idx in self.meta.episodes:
        #     action_config = self.meta.episodes[ep_idx].get("action_config", [])
        #     # 找出当前帧属于哪个动作
        #     current_frame = item["frame_index"].item()  # 假设 hf_dataset 有 'frame_index'
        #     action_text_to_use = None
        #     for action in action_config:
        #         if action["start_frame"] <= current_frame <= action["end_frame"]:
        #             at = action.get("action_text")
        #             eat = action.get("english_action_text")
        #             if at and eat:
        #                 action_text_to_use = eat  # 两者都有，优先 english_action_text
        #             else:
        #                 action_text_to_use = at or eat  # 取存在的那个
        #             break

        #     # 如果找不到，就用 task 替代
        #     if action_text_to_use is None:
        #         action_text_to_use = item["task"]

        #     item["english_action_text"] = action_text_to_use
        # ------------------- 获得拼接动作文本 -------------------
        if not self.COMPUTE_NORM:
            if ep_idx in self._episode_action_cache:
                # 直接使用缓存
                item["english_action_text"] = self._episode_action_cache[ep_idx]
                # print(f"Using cached action text for episode {ep_idx}")
            else:
                # 拼接当前 episode 的动作文本
                action_text_list = []
                if hasattr(self.meta, "episodes") and ep_idx in self.meta.episodes:
                    ...
                    for action in action_config:
                        ...
                        at = action.get("action_text")
                        if eat:
                            action_text_list.append(eat)
                        elif at:
                            action_text_list.append(at)

                if not action_text_list:
                    action_text_list.append(item["task"])

                full_action_text = " ".join(action_text_list)
                # 缓存
                self._episode_action_cache[ep_idx] = full_action_text
                item["english_action_text"] = full_action_text
        return item
